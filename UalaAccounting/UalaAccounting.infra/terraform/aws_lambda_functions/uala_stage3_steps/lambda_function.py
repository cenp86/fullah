...
secrets = json.loads(secrets_client.get_secret_value(SecretId=os.environ['SECRETS_ARN']).get('SecretString'))
...
def get_account_details(loanAccountId):
    ...
    api_url = f"https://mbucarlosn.sandbox.mambu.com/api/loans/{loanAccountId}"
    data = ""
...
def apply_interest(loanAccountId, dateTime):
    ...
    api_url = f"https://mbucarlosn.sandbox.mambu.com/api/loans/{loanAccountId}:applyInterest"
